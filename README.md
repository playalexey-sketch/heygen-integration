...
## ⚡ Локальный Qwen + фабрика агентов (Bionic / LM Studio)

Если вы скачали **Qwen2.5-1.5B-Instruct-GGUF** и хотите человеческий чат плюс
несколько агентов, которые сами между собой переписываются, а вы только следите:

1. Импортируйте GGUF в LM Studio / Bionic (`lms import путь\к\файлу.gguf --copy`).
2. В Bionic откройте **Work project** — это обычный чат с одним агентом.
3. Bionic **не умеет фабрику ролей**. Для этого:
...
На Windows можно дважды кликнуть `run_factory.bat`.

Перед этим поднимите модель как OpenAI-сервер (порт 1234):
...
На пульте: создаёте агентов → собираете фабрику → жмёте «Далее» на каждый ход
или отпускаете их сами. Подробный разбор (и честные ограничения 1.5B) —
в [`factory/README.md`](factory/README.md).
