...
# Показывается в Telegram при вводе "/" в чате с ботом.
BOT_COMMANDS = [
    ("start", "запустить сценарий (для клиентов)"),
    ("help", "справочник всех команд"),
    ("admin", "админ-меню (кнопки)"),
    ("stages", "список этапов"),
    ("add", "добавить этап (мастер)"),
    ("edit", "редактировать этап: /edit N"),
    ("del", "удалить этап: /del N"),
    ("up", "этап выше: /up N"),
    ("down", "этап ниже: /down N"),
    ("on", "включить этап: /on N"),
    ("off", "выключить этап: /off N"),
    ("test", "тест сценария (сразу)"),
    ("test_live", "тест сценария (с задержками)"),
    ("rules", "правила и ссылки для клиентов"),
    ("addrule", "добавить правило (мастер)"),
    ("delrule", "удалить правило: /delrule N"),
    ("onrule", "включить правило: /onrule N"),
    ("offrule", "выключить правило: /offrule N"),
    ("media", "список медиа-файлов"),
    ("upmedia", "загрузить файл (следующим сообщением)"),
...
ADMIN_HELP = (
    ...
    "⚙️ МЕНЮ\n"
    "/admin — админ-меню (кнопки)\n"
    "/setmenu — обновить меню «/» бота\n"
    "/cancel — отменить текущий мастер\n"
    "\n"
    "📋 ЭТАПЫ (уходят клиенту по порядку после /start)\n"
    "/stages — список этапов\n"
    "/add — добавить: задержка → тип → контент (медиа — файлом)\n"
    "/edit N — редактировать этап N\n"
    "/del N — удалить\n"
    "/up N, /down N — порядок\n"
    "/on N, /off N — вкл/выкл\n"
    "/test — прогнать сценарий сразу\n"
    "/test_live — прогнать с реальными задержками\n"
    "\n"
    "🔑 ПРАВИЛА И ССЫЛКИ (ключевое слово → контент)\n"
    "/rules — список правил + готовые ссылки (т.ч. для соцсетей)\n"
    "/addrule — добавить правило\n"
    "/delrule N, /onrule N, /offrule N — управление\n"
    "\n"
    "🎬 МЕДИА (видео/аудио/картинки/файлы)\n"
    "/media — список файлов\n"
    "/upmedia — загрузить файл следующим сообщением\n"
    "\n"
    "🔌 ПОДКЛЮЧЕНИЕ (VPN)\n"
...
async def set_bot_menu(bot: Bot) -> None:
    """Зарегистрировать меню команд (показывается при вводе «/» в чате)."""
...
async def cmd_setmenu(message: Message, admin: AdminService):
    if not await guard_admin(message, admin):
        return
    try:
        await set_bot_menu(message.bot)
        await message.answer(f"✅ Меню команд обновлено: {len(BOT_COMMANDS)} команд. "
                             "Введите «/» в чате — они появятся списком с подсказками.")
    except Exception as e:
        await message.answer(f"⚠️ Не удалось обновить меню (нет связи с Telegram): {e}")
