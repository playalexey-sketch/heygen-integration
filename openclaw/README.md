...
![Схема: OpenClaw + Telegram + локальная модель без Ollama](scheme.png)

Исходник — [`scheme.svg`](scheme.svg) (векторный, можно править и печатать).

### Кто есть кто: Ollama ≠ Llama

Уточню важное, раз вы об этом спросили. Под капотом Ollama находится **не «Llama»**, а движок
**llama.cpp**. Названия похожи, но это три разные вещи:

| Что | Роль | Аналогия |
|---|---|---|
| **Llama / Qwen / Mistral** | **модель** — файл весов `.gguf`, сам по себе не запускается | видеофайл |
| **llama.cpp** (`llama-server`) | **движок** — программа, которая крутит модель. MIT, открытый код | видеоплеер |
| **Ollama** | **обёртка** над llama.cpp: удобный CLI, свой реестр моделей | плеер с красивой оболочкой |
| **LM Studio** | **GUI-обёртка** над тем же llama.cpp | то же, но с окошками |

Поэтому «убрать Ollama» не значит остаться без Llama. Мы просто берём тот самый движок,
который был у Ollama внутри, и подключаем его к OpenClaw напрямую — на один слой меньше,
на один источник поломок меньше.

### Что за что отвечает

| # | Компонент | Где работает | Роль |
|---|---|---|---|
| 1 | **Telegram** | ваш телефон/ПК | вы пишете боту, ставить ничего не нужно |
| 2 | **api.telegram.org** | облако Telegram | доставляет сообщения, нужен токен от @BotFather |
| 3 | **OpenClaw** | ваш компьютер, порт 18789 | шлюз: сессии, права, инструменты, скиллы |
| 4 | **llama-server** | ваш компьютер, порт 8080 | движок: считает модель, отдаёт `/v1/chat/completions` |
| — | **модель `.gguf`** | файл на диске (~5 ГБ) | сами веса, скачиваются один раз |

Наружу уходит только переписка через Telegram — сам текст обрабатывается локально,
в облако модели ничего не отправляется.
...
| Файл | Назначение |
|---|---|
| `scheme.png` / `scheme.svg` | схема всей связки одной картинкой |
